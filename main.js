...
function parseWordsFromCsvText(t) {
	...
	for (const raw of lines) {
		...
		if (!line) continue;
		// Split by comma OR whitespace (same as before)
		const cells = line.split(/,|\s+/);
		for (let c of cells) {
			if (!c) continue;
			// 1) Trim + lowercase
			c = c.trim().toLowerCase();
			// 2) Normalize smart quotes to regular quotes
			//    ‘ ’  → '
			//    “ ”  → "
			c = c
				.replace(/[\u2018\u2019]/g, "'")
				.replace(/[\u201C\u201D]/g, '"');
			// 3) Strip leading/trailing single/double quotes
			//    e.g. "raise"  -> raise
			//          'hello'  -> hello
			//          ""word"" -> word
			c = c.replace(/^["']+|["']+$/g, '');

			// 4) Now we only accept *exactly* 5 letters a–z.
			//    Anything with digits, hyphens, extra punctuation, etc. is dropped.
			if (!/^[a-z]{5}$/.test(c)) {
				continue;
			}
			// 5) De-duplicate
...
function setFilterMode(mode) {
	...
	if (currentMode === 'adv' && mode === 'basic') {
		showToast("Some advanced filters may not be reflected in Basic Mode (e.g., repeated-letter counts). Use Advanced Mode → Pattern Search for exact filtering.", "warn");
