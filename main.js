...
function refreshPreviousAnswersControls(){
	const cb = byId('excludePreviousAnswers');
	const modeSel = byId('previousAnswersMode');
	const valueWrap = byId('previousAnswersValueWrap');
	const valueInput = byId('previousAnswersValue');
	const pa = state.previousAnswers;

	if (!cb || !modeSel || !valueWrap || !valueInput) return;

	const ready = !!pa.loaded && !pa.loadError;
	const enabled = ready && !!pa.enabled;

	cb.disabled = !ready;
	modeSel.disabled = !enabled;
	valueInput.disabled = !enabled || pa.mode !== 'lastN';

	valueWrap.style.display = (enabled && pa.mode === 'lastN') ? '' : 'none';

	modeSel.value = pa.mode || 'all';
...
function applyPreviousAnswersFilter(words){
	...
	if (!pa.enabled || !pa.loaded || !pa.activeSet.size) return words;
    ...
    clearTop.addEventListener('click', (e) => {
      ...
      if (appMode === 'wordle'){
        ...
        if (state.previousAnswers){
          state.previousAnswers.enabled = false;
          state.previousAnswers.mode = 'all';
          state.previousAnswers.value = 365;
          state.previousAnswers.activeSet.clear();
        }

        const prevCb = byId('excludePreviousAnswers');
        if (prevCb) prevCb.checked = false;

        const prevModeSel = byId('previousAnswersMode');
        if (prevModeSel) prevModeSel.value = 'all';
...
function lockUI(yes){
	...
	['wordListSelect','excludePreviousAnswers','previousAnswersMode','previousAnswersValue','modeSelect','fwHardMode','fwGuessPool','hardMode','maxCand','maxPool','deepEstepThreshold','poolSelect','deepSearch','analyzeInput'].forEach(id=>{
		const el = byId(id);
		if (!el) return;
		el.disabled = yes;
	});
...
function wire(){
	...
	function reapplyPreviousAnswersFilter(){
		rebuildPreviousAnswersActiveSet();
		updatePreviousAnswersUI();
	...
	if (prevCb) {
		prevCb.onchange = e => {
			state.previousAnswers.enabled = !!e.target.checked;
			reapplyPreviousAnswersFilter();
		};
	}

	if (prevModeSel) {
		prevModeSel.onchange = e => {
...
function clearRecoAndAnalyzeResults(){
  ...
  if (inp) inp.value = inp.value; // no-op (keeps user's input)
