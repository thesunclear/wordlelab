...
function getFirstGuessJsonFile(){
  ...
  if (!ansKey) return { error: "Please choose a Word List at the top of the page: 2,309 Answer List or 3,207 Answer List." };
...
document.addEventListener('DOMContentLoaded', async function(){
	...
	try{
		const base = await fetchCsv('wordle_solutions_3207.csv');
		state.base3207 = base.slice();
		loadWordsFromArray(base);
		const sel = byId('wordListSelect');
		if (sel) sel.value = 'wl3207';   // sync UI with default
	} catch(e) {
		console.error(e);
	}
});
