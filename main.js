...
function initWordleModeUI(){
  ...
  if (addBtn && inp){
    addBtn.addEventListener('click', () => {
      ...
      if (!isValidGuessText(g)){
        showToast('Please enter a 5-letter word (A–Z).', 'warn');
        return;
      }
      // Add row (default: not applied yet)
      wordleState.rows.push({ guess: g, pattern: [0,0,0,0,0], applied: false });
      inp.value = '';
      renderWordleRows();
      // Do not filter until user applies, to avoid accidental "all gray" constraint
      // applyWordleFiltersAndRender();
      
      // No filtering happens until user clicks Apply on a row.
      const st = byId('wordleStatus');
      if (st) st.textContent = 'Added a guess (not applied yet).';
    });
