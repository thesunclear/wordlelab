		...
		d.onclick = () => {
			...
			if (kind === 'gInc') {
				...
				toggle(state.globalInclude, state.globalExclude, letter);
					if (hadGray) {
						toast(`"${letter.toUpperCase()}" was removed from Global Exclude.`);
					}
			} else if (kind === 'gExc') {
				// Global Exclude:
				...
				if (!wasExcluded && isNowExcluded) {
					...
					if (cleared || wasIncluded) {
						toast(`"${letter.toUpperCase()}" was removed from all Global and Position Includes.`);
					}
				}
			} else if (kind === 'pInc') {
				...
				if (!wasIncluded && isNowIncluded) {
					if (hadYellow && hadGray) {
						...
						toast(`"${letter.toUpperCase()}" was removed from both Global Exclude and Position Exclude (Position ${state.activePos + 1}).`);
					} else if (hadGray && !hadYellow) {
						state.globalExclude.delete(letter);
						toast(`"${letter.toUpperCase()}" was removed from Global Exclude.`);
					} else if (!hadGray && hadYellow) {
						toast(`"${letter.toUpperCase()}" was removed from Position Exclude (Position ${state.activePos + 1}).`);
					}
				}
				/* if (!wasIncluded && isNowIncluded) {
					const msgs = [];
				...
				if (wasIncluded) {
					toast(`"${letter.toUpperCase()}" was removed from Position Include (Position ${state.activePos + 1}).`);
				}
				...
				if (slot.include.size === 1 && slot.include.has(letter)) {
					...
					if (slot.exclude.delete(letter)) {
						toast(`"${letter.toUpperCase()}" was removed from Yellow Letter (Position ${state.activePos + 1}).`);
					}
					if (state.globalExclude.delete(letter)) {
						toast(`"${letter.toUpperCase()}" was removed from Gray Letter.`);
					}
				}
				...
				if (slot.exclude.has(letter)) {
					...
					if (wasIncluded) {
						...
						toast(`"${letter.toUpperCase()}" was removed from Green Letter (Position ${state.activePos + 1}).`);
					}
					if (hadGray) {
						state.globalExclude.delete(letter);
						toast(`"${letter.toUpperCase()}" was removed from Gray Letter.`);
				...
				if (state.globalExclude.has(letter)) {
					...
					toast(`"${letter.toUpperCase()}" was removed from all Green and Yellow Letters.`);
				}
			}
