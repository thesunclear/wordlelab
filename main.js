...
/* Shallow E[steps] with an extra lookahead level:
...
*/
function EstepsOneLookaheadLocalShallow(S, g) {
  const n = S.length;
...
/* Medium E[steps] with an extra lookahead level:
   - Level 1: g (the candidate we are evaluating now)
   - Level 2: choose best g2 inside each bucket after g
   - Level 3: choose best g3 inside each bucket after g2
   - Level 4: choose best g4 inside each bucket after g3
   ...
   - The “leaf” buckets add a small extra cost depending on size.
*/
function EstepsOneLookaheadLocalMedium(S, g) {
  const n = S.length;
  const parts1 = partitionByPattern(S, g);
  let est = 0;
...
const HIDDEN_LEVEL3_LIMIT = 33;

function EstepsOneLookaheadLocalDeep(S, g) {
    const n = S.length;

    if (n > HIDDEN_LEVEL2_LIMIT) {
        return EstepsOneLookaheadLocalShallow(S, g);
    } 
    else if (n > HIDDEN_LEVEL3_LIMIT) {
        return EstepsOneLookaheadLocalMedium(S, g);
    } 
    else {
...
/* Shallow E[steps] with an extra lookahead level:
   ...
   - The “leaf” buckets add a small extra cost depending on size.
*/
function EstepsOneLookaheadShallow(S, g) {
  const n = S.length;
  const parts1 = partitionByPattern(S, g);
  let est = 0;
...
/* Medium E[steps] with an extra lookahead level:
   ...
   - The “leaf” buckets add a small extra cost depending on size.
*/
function EstepsOneLookaheadMedium(S, g) {
  const n = S.length;
  const parts1 = partitionByPattern(S, g);
  let est = 0;
...
function EstepsOneLookaheadDeep(S, g) {
    ...
    if (n > HIDDEN_LEVEL2_LIMIT) {
        return EstepsOneLookaheadShallow(S, g);
    } 
    else if (n > HIDDEN_LEVEL3_LIMIT) {
        return EstepsOneLookaheadMedium(S, g);
    } 
    else {
