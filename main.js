...
if (showBadge && pa.activeSet.has(w)) {
  const rec = getPreviousAnswerTooltipRecord(w);
  const tooltip = rec && Number.isInteger(rec.number)
    ? `Past official answer (#${rec.number})`
    : 'Past official answer';

  li.title = tooltip;
  li.innerHTML = `${w.toUpperCase()} <span class="pa-badge" title="${tooltip}">past</span>`;
} else {
