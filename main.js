...
function applyWordleFilters(){
  // Base candidates are always state.all (built from top Word List selection)
  let arr = state.all || [];
...
async function getExternalPool(){
	...
	if(sel==='2302')  return state.base2302.length? state.base2302 : state.all;
...
function lockUI(yes){
	...
	['wordListSelect','modeSelect','fwHardMode','fwGuessPool','hardMode','maxCand','maxPool','deepEstepThreshold','poolSelect','deepSearch','analyzeInput'].forEach(id=>{
		const el = byId(id);
		if (!el) return;
		el.disabled = yes;
	});
...
document.addEventListener('DOMContentLoaded', async function(){
	...
	try{
		const base = await fetchCsv('wordle_solutions_3200.csv');
