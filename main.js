...
/* ===== Numeric formatting: show 3 decimals, hover full precision ===== */
/* ===== Numeric formatting: show 3 decimals, hover ~5 decimals (optional quantize) ===== */
const HOVER_DECIMALS = 5;

// If set to a positive integer, hover titles will be snapped to multiples of 1 / denom.
// This is useful for "exact E[steps]" under uniform answer probabilities.
...
function quantizeToDenom(x, denom) {
  if (!denom || denom <= 0) return x;
...
function fmt3WithHover(x) {
  ...
  // Display is always 3 decimals (UI consistency)
  const text = x.toFixed(3);

  // Hover: by default show 5 decimals (enough to distinguish meaningful differences)
  // If HOVER_QUANTIZE_DENOM is set, snap to 1/denom grid first to avoid fake float differences.
  const snapped = quantizeToDenom(x, HOVER_QUANTIZE_DENOM);
  const title = snapped.toFixed(HOVER_DECIMALS);

  return { text, title };
}

function tdNum3(x) {
  const f = fmt3WithHover(x);
  return f.title
    ? `<td title="${escapeHtmlAttr(f.title)}">${f.text}</td>`
    : `<td>${f.text}</td>`;
}
...
(function(){
	...
	function renderAnalyzeRow(result){
		...
		tr.innerHTML = `
			...
			${tdNum3(result.esteps)}
			${tdNum3(result.entropy)}
			${tdNum3(result.expected)}
