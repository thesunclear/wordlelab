#!/usr/bin/env node
// dp_top_blocks.js
// Exact hard-mode E[steps] for a block of high-entropy starters.
//
// Usage:
...
//
// We *do not* search globally optimal first guess here. Instead, for each
// chosen starter g we compute E[steps] if you *must* play g first, and then
...
for (const item of selectedStarters) {
  ...
  console.log(`[${timeStr()}]  #${startRank + processed}  ${word.toUpperCase()}  H=${H.toFixed(6)}  E=${E.toFixed(6)}`);
...
const outName = `dp_top_block_${path.basename(WORD_FILE, path.extname(WORD_FILE))}_K${BLOCK_SIZE}_B${BLOCK_INDEX}.json`;
